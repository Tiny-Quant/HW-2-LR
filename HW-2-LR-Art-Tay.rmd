...
linear model of a particular listings price.
...
most imputation algorithms. All nominal predictor were transformed into $n-1$
...
there are methods for including nonlinear relationship into a linear regression
model. Frank Harrell in his book \emph{Regression Modeling Strategies},
recommends treating all numeric predictors are natural cubic splines to capture
non-linear relationships. Natural cubic splines are piecewise third degree
piecewise polynomial transformations of predictors with added smoothness restrictions.
Than can be past to the linear model via 3\* basis functions per variable
(marked by _ns\#). \* Not all variables produced 3 uncorrelated basis functions.
...
influential in the model base on Cook's distance. Removing points is unlikely
...
in price for both the training and testing data. There is no significant drop
...
Given the extreme non-linearity of the data and the unequal variance of the
